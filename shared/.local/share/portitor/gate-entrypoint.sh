#!/bin/sh
# portitor gate entrypoint — dot-maintained reproduction of portitor's runtime
# contract (its repo deploy/entrypoint.sh). Kept in dot so the gate image builds
# with NO portitor checkout: the binary is a verified release download and this
# script supplies the sshd + forced-command wiring the container needs. If
# portitor changes its entrypoint contract, re-sync this file from that release.
#
# A git-over-SSH gateway + GitHub action mediator.
#
# Inputs (env):
